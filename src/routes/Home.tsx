...
function Home() {
	...
	return (
		<div className="h-screen max-w-2xl flex flex-col pt-16 mx-auto">
			<div className=" px-6 md:px-10 mb-6 flex justify-center">
				<TaskForm />
			</div>
			<div className="h-full bg-primary-700 rounded-t-xl flex flex-col overflow-hidden mt-6">
				...
				<div className=" h-full w-full bg-primary-200 rounded-t-xl px-1 flex overflow-hidden">
					{tasks.length ? (
						<div className="h-full w-full grid grid-cols-2 lg:grid-cols-3 gap-2 overflow-scroll p-5 md:p-8">
							{tasks.map((task: Task) => (
								<div
									className="w-full h-40 lg:h-48 bg-white rounded-lg shadow-md p-3 flex flex-col"
									key={task.id}
								>
